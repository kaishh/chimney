...
enough. What you can do instead is to automate the mapping process with
a help from Chimney library.
...
In some codebases a problem of domain model versioning starts to appear. In fact there
are number of ways how to solve it that differ when it comes to level of complexity
and provided guarantees. One of the simplest approach that often seems to be sufficient
is to maintain evolving versions of the domain models in different packages, like
...
val v2User: v2.User = v1User.into[v2.User]
  .withFieldComputed(_.addresses, userV1 => List(v2.Address(userV1.street, userV1.postalCode)))
  .transform
...
It's a common practice to use Protocol Buffers, Avro, or similar serialization mechanisms to
exchange data between the external systems. But usually defined schemas for such data transfer
objects is not such type-safe, like in internal domain models. For following example protobuf
definition:
...
will be generated by ScalaPB. However a good practice is not to work on such DTOs directly, but to
define our internal, type-rich domain counterpart along with the business logic. Such representation
will be usually slightly different, involving value classes or tagged types instead of raw primitive
types, like:
...
package domain {
  ...
  case class Person(id: PersonId, name: PersonName, email: EmailAddress)
}
```

As you may expect, Chimney helps also in that case by saving you from manual wrapping/unwrapping
every field in type-safe domain representation.

```scala
import io.scalaland.chimney.dsl._

val protoPerson = proto.Person(303, "Mike", "mike@example.com")
// protoPerson: proto.Person = Person(303,Mike,mike@example.com)

val domainPerson = protoPerson.transformInto[domain.Person]
// domainPerson: domain.Person = Person(PersonId(303),PersonName(Mike),EmailAddress(mike@example.com))

val protoPerson2 = domainPerson.transformInto[proto.Person]
// protoPerson2: proto.Person = Person(303,Mike,mike@example.com)
```
